...
import { useUpdatePaymentPlanMutation } from '../../../../redux/paymentApi/paymentApi'

interface FormErrors {
  title?: string
}

function EditPaymentInvoice() {
  const { id } = useParams()
  ...
  const [updatePaymentPlan, { isLoading }] = useUpdatePaymentPlanMutation()
  ...
  const handleSavePost = async () => {
    ...
    try {
      ...
      formData.append("data", JSON.stringify({ name: title }))
      if (files[0]) formData.append("payment_plan_file", files[0])

      const response = await updatePaymentPlan({ id, data: formData }).unwrap()

      if (response?.success) {
        toast.success(response?.message || "Payment plan updated successfully!")
        setErrors({})
      }
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to update payment plan")
    }
  }
        ...
        <FormalCard header={
          <div>
            ...
            <p className="text-sm text-[#B0B0B0] font-nunito-semibold-italic">
              Update the Payment Plan details for the selected property.
            </p>
          </div>
        }>
          <div className="p-4 space-y-8">
            ...
            {state?.file_url && files.length === 0 && (
              <Field>
                <FieldLabel className="text-base font-medium text-gray-600 block">
                  Current File
                </FieldLabel>
                <div className="flex items-center justify-between p-3 border border-gray-200 rounded-xl bg-gray-50">
                  <span className="text-sm text-gray-600 truncate max-w-xs">{state?.name || 'Current file'}</span>
                  <a
                    href={state.file_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-brand underline ml-2 whitespace-nowrap"
                  >
                    View file
                  </a>
                </div>
            ...
            <Field>
              <FieldLabel className="text-base font-medium text-gray-600 block">
                {state?.file_url ? 'Replace File (optional)' : 'Upload Payment Plan File'}
              </FieldLabel>
              <FileUpload
                value={files}
                onChange={setFiles}
...
export default EditPaymentInvoice
