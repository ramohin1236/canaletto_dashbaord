...
import { useDeleteSingleAllPropertyFilesMutation, useGetSingleAllPropertyFilesQuery } from '../../../../redux/property/propertyApis'
...
function propertyFiles() {
  ...
  const { data: allPropertyFiles, isLoading } = useGetSingleAllPropertyFilesQuery(propertyId)
  const [deleteSingleAllPropertyFiles] = useDeleteSingleAllPropertyFilesMutation()
  ...
  const renderAction = (id: string) => (
    <Button
      onClick={() => navigateToRouter(`/property-files/create`, id)}
      className="bg-brand text-white"
      variant="outline"
    >
      <HugeiconsIcon icon={AddSquareIcon} /> Add New File
    </Button>
  )
  ...
  return (
    <PageLayout title='The Wilds Project / Property Files'>
      <PageContent>
        ...
        <FormalCard header='Property Files' action={renderAction(sate?.id)}>
